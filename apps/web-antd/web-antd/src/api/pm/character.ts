...
const baseUrl = '/api/v1/admin/characters';

function filterItems(items: PmCharacterApi.CharacterItem[], params: Record<string, any>) {
  const keyword = String(params.keyword || '').trim().toLowerCase();
  return items.filter((item) => {
    if (params.characterTypeCode && item.characterTypeCode !== params.characterTypeCode) {
      return false;
    }
...
export async function updateCharacter(id: string, payload: Record<string, any>) {
  try {
    ...
    return (await updateCharacterMock(id, payload)) as PmCharacterApi.CharacterItem;
  }
}
